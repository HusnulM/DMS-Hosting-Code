...
class DocumentV3Controller extends Controller
{
    public function index(){
        $doctypes  = DB::table('doctypes')->where('doctype', 'Work Standard')->get();
        ...
        return view('transaction.document.v3.index', ['doctypes' => $doctypes, 'doclevels' => $doclevels, 'docareas' => $docareas, 'ipdapi' => $ipdapi]);
    }
    ...
    public function save(Request $req){
        ...
        try{
            ...
            $dcnNumber = generateDcnNumber('WS');
            ...
            return Redirect::to("/document/v3")->withSuccess('New Document Created With Number '. $dcnNumber);
        } catch(\Exception $e){
            DB::rollBack();
    ...
    public function saveNewDocVersion($id, Request $req){
        ...
        try{
            ...
            if(sizeof($wfapproval) > 0){
                ...
                $doctype = DB::table('doctypes')->where('id', $req['doctype'])->first();
                // return Redirect::to("/transaction/document")
                return Redirect::to("/document/v3")
                ->withError('Approval Workflow Not Maintained Yet for user '. Auth::user()->username . ' in document type ' . $doctype->doctype);
            }
